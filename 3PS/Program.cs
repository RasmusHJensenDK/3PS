...
namespace _3PS
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach(Field fld in board.GetField())
            {
                Console.WriteLine(fld.GetFieldsForTokens().ToString());
            }
            ...
            Console.WriteLine("Drawing .. " + board.drawField().GetPieceToken().ToString() + board.GetField().Length);
            Console.WriteLine("Drawing .. " + board.drawField().GetPieceToken().ToString());
            Console.WriteLine("Drawing .. " + board.drawField().GetPieceToken().ToString());
            Console.WriteLine("Drawing .. " + board.drawField().GetPieceToken().ToString());

            board.SetField(3, field2);
